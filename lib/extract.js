...
export function classifyFonts(fonts) {
  ...
  for (const f of fonts) {
    ...
    if (/(mono|code|courier|consolas)/.test(n)) {
      if (!stacks.mono) stacks.mono = f.name;
    } else if (/(serif|georgia|garamond|playfair|times)/.test(n) && !/sans/.test(n)) {
      if (!stacks.serif) stacks.serif = f.name;
    } else if (!stacks.sans) {
      stacks.sans = f.name;
    }
  }
